  ...
  implements Storage<ModelWithoutId>
{
  ...
  async doUpsert(obj: ModelWithoutId): Promise<number> {
    ...
    if (JSON.stringify(this.rows[position]) !== JSON.stringify(obj)) {
      // console.log('UPDATING ROW', position, this.rows[position], obj);
    }
    this.rows[position] = obj;
    return position;
  }
